...
select e.course_name from students s right join enrollments e on s.student_id=e.student_id;
...
select s.student_name,count(*) from students s inner join enrollments e on s.student_id=e.student_id group by s.student_id;

select s.student_name,sum(e.fee) TotalFees from students s inner join enrollments e on s.student_id=e.student_id group by e.student_id;

select s.student_name,count(*) as TotalCourses from students s inner join enrollments e on s.student_id=e.student_id group by s.student_id having count(*)>1;

select e.trainer,sum(e.fee) TotalFees from students s inner join enrollments e on s.student_id=e.student_id group by e.trainer having TotalFees>10000;
...
having total_fee_paid>5000 order by total_fee_paid desc;
